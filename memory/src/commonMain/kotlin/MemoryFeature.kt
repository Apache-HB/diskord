...
/** The [MemoryProvider] is used by a [MemoryFeature] to produce new [memories][Memory] from a specific [Entity]. */
...
abstract class MemoryFeature<M : Memory> : BotFeature {

    /** The [logger] for this [BotFeature]. Set to off by default. */
    protected val logger: Logger = Logger().apply { level = LogLevel.OFF }

    /** The memory provider which will produce memories for this [MemoryFeature]. */
    ...
    companion object {
        /** The name of the memory feature. */
...
/**
 * Strife's built-in [MemoryFeature] implementation.
 *
 * !NOTE! [StrifeMemoryFeature] is NOT persistent across runs. These memories do not stay between launches of the bot
 * . To add persistence to Strife, you will need to implement your own [MemoryFeature].
 *
 ...
 */
class StrifeMemoryFeature<M : Memory> internal constructor(
    override val provider: MemoryProvider<M>,
    override val removeOnExit: Boolean,
    existingMemories: Map<Long, M> = emptyMap()
) : MemoryFeature<M>() {

    /** The backing field containing all memories for this feature. */
    ...
    /**
     * The [BotFeatureProvider] of the [StrifeMemoryFeature].
     *
     * @property removeOnExit See [MemoryFeature.removeOnExit].
     * @property existingMemories Any pre-existing memories to be added to the [StrifeMemoryFeature].
     * @property memoryProvider The MemoryProvider for the [provided][provide] [StrifeMemoryFeature].
     */
    ...
    ) : BotFeatureProvider<StrifeMemoryFeature<M>> {
        override fun provide(): StrifeMemoryFeature<M> =
            StrifeMemoryFeature(MemoryProvider<M>().apply(memoryProvider), true, existingMemories)
    }
