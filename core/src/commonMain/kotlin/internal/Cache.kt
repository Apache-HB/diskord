...
internal expect class WeakCache<K, V>() : MutableMap<K, V>

internal class CombinationCache<K, V> : MutableMap<K, V> {
    private val lruCache = LruCache<K, V>()
    private val weakCache = WeakCache<K, V>()

    override val size: Int get() = lruCache.size + weakCache.size
